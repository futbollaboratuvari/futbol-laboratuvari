(() => {
  ...
  const bindMembershipReturn = () => {
    const panel = document.getElementById("membership-payment-panel");
    if (!panel) return;
    const output = panel.querySelector("[data-membership-output]");
    if (output && !output.querySelector(".membership-to-analysis")) {
      output.insertAdjacentHTML("beforeend", `<br><a class="membership-to-analysis" href="#premium-analysis-panel">Özel Analiz paneline dön</a>`);
    }
  };

  const render = () => {
    style();
    bindMembershipReturn();
  ...
  document.addEventListener("click", (e) => {
    if (e.target.closest?.("#membership-payment-panel [data-plan]")) setTimeout(render, 160);
