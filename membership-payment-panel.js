  ...
  const render = (plans) => {
    ...
    shell.innerHTML = `
      <div class="membership-head">
        ...
        <span class="membership-badge">💳 PayTR Hazırlığı</span>
      </div>
      ...
      <button class="membership-start" type="button" data-payment-start disabled>Ödeme Altyapısını Başlat</button>
      <div class="membership-flow">
        <div class="membership-step"><b>1. Üyelik</b><span>Kullanıcı hesap oluşturur ve paket seçer.</span></div>
        <div class="membership-step"><b>2. Ödeme</b><span>PayTR ödeme ekranına yönlenir.</span></div>
        <div class="membership-step"><b>3. Onay</b><span>PayTR bildirimi backend'e gelir.</span></div>
        <div class="membership-step"><b>4. Panel</b><span>Ödeme başarılıysa premium panel açılır.</span></div>
      </div>
      <div class="membership-output" data-membership-output><strong>Durum:</strong> Paket seç ve üye bilgilerini doldur. Backend bağlanınca ödeme başlatılacak.</div>
      <p class="membership-small">Kart bilgisi sitede tutulmayacak. PayTR ödeme alacak, callback backend'e gelecek, backend üyeliği aktif edecek.</p>
    `;
    ...
    shell.querySelectorAll("[data-plan]").forEach((button) => {
      button.addEventListener("click", () => {
        ...
        output.innerHTML = `<strong>Seçilen paket:</strong> ${esc(selectedPlan?.name || "-")}<br><strong>Sonraki adım:</strong> Üye bilgilerini doldur. PayTR backend bağlandığında ödeme ekranı açılacak.`;
        refreshButton();
      });
    });
    ...
    startButton.addEventListener("click", () => {
      const request = {
        ...
        status: "backend_waiting",
        created_at: new Date().toISOString(),
      };
      localStorage.setItem("fl_pending_payment_request", JSON.stringify(request));
      output.innerHTML = `<strong>Ödeme isteği hazırlandı.</strong><br>Backend bağlandığında bu bilgiler <code>${PAYMENT_ENDPOINT}</code> adresine gönderilecek ve PayTR ödeme ekranı açılacak.`;
    });
  };
