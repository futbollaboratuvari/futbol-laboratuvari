(() => {
  ...
  const render = (plans) => {
    ...
    shell.innerHTML = `
      ...
      <div class="membership-output" data-membership-output><strong>Durum:</strong> ${selected?.name ? `${esc(selected.name)} seçildi.` : "Paket seç."}<br><strong>Kod girişi:</strong> Kodunu Özel Analiz panelindeki Üye / kurucu kodu kutusuna yaz.<br><a class="membership-return" href="#premium-analysis-panel">Özel Analiz paneline dön</a></div>
    ...
    shell.querySelectorAll("[data-plan]").forEach((button) => {
      button.addEventListener("click", () => {
        ...
        output.innerHTML = `<strong>Seçilen paket:</strong> ${esc(selectedPlan?.name || "-")}<br><strong>Kod girişi:</strong> Özel Analiz paneline dön, Ücretli Üye Alanı yazısına tıkla ve kodunu gir.<br><a class="membership-return" href="#premium-analysis-panel">Özel Analiz paneline dön</a>`;
