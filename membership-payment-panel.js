(() => {
  ...
  const injectStyle = () => {
    ...
    style.textContent = `
      ...
      @media(max-width:900px){.membership-grid,.membership-flow,.membership-form,.membership-actions{grid-template-columns:1fr}.membership-card.pro{transform:none}.membership-card.selected{transform:none}.membership-head{flex-direction:column}.membership-badge{width:max-content}}@media(max-width:560px){.membership-shell{margin:18px 14px 0;padding:14px}}
    `;
    ...
    shell.innerHTML = `
      ...
      <div class="membership-grid">
        ${visiblePlans.map((plan) => {
          ...
          return `
          <article class="membership-card ${tone.className}" data-plan-card="${esc(plan.id)}">
            ...
            <button class="membership-pay" type="button" data-plan="${esc(plan.id)}" aria-pressed="false">${esc(plan.cta || "Paketi Seç")}</button>
          </article>`;
        }).join("")}
      </div>
