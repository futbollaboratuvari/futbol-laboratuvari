...
const stat = (m) => ({
  home_possession: pick(m, ['homePossession','home_possession','ev_topla_oynama','home_ball_possession']),
  away_possession: pick(m, ['awayPossession','away_possession','dep_topla_oynama','away_ball_possession']),
  home_passes: pick(m, ['homePasses','home_passes','ev_pas','home_total_passes']),
  away_passes: pick(m, ['awayPasses','away_passes','dep_pas','away_total_passes']),
  home_shots: pick(m, ['homeShots','home_shots','ev_sut','home_total_shots']),
  away_shots: pick(m, ['awayShots','away_shots','dep_sut','away_total_shots']),
  home_free_kicks: pick(m, ['homeFreeKicks','home_free_kicks','ev_serbest_vurus','home_freekicks']),
  away_free_kicks: pick(m, ['awayFreeKicks','away_free_kicks','dep_serbest_vurus','away_freekicks']),
  home_corners: pick(m, ['homeCorners','home_corners','ev_korner']),
  away_corners: pick(m, ['awayCorners','away_corners','dep_korner']),
  home_yellow_cards: pick(m, ['homeYellowCards','home_yellow_cards','ev_sari_kart']),
  away_yellow_cards: pick(m, ['awayYellowCards','away_yellow_cards','dep_sari_kart']),
  home_red_cards: pick(m, ['homeRedCards','home_red_cards','ev_kirmizi_kart']),
  away_red_cards: pick(m, ['awayRedCards','away_red_cards','dep_kirmizi_kart'])
});
const dominant = (s) => {
  let h = 0, a = 0;
  [['possession',1],['passes',1],['shots',2],['free_kicks',1],['corners',1]].forEach(([k,w]) => { const x=s[`home_${k}`], y=s[`away_${k}`]; if (x===null||y===null) return; if (x>y) h+=w; if (y>x) a+=w; });
  const hr = s.home_red_cards || 0, ar = s.away_red_cards || 0;
  const hy = s.home_yellow_cards || 0, ay = s.away_yellow_cards || 0;
  if (hr > ar) h -= 2;
  if (ar > hr) a -= 2;
  if (hy > ay) h -= 1;
  if (ay > hy) a -= 1;
...
const statFields = ['possession','passes','shots','free_kicks','corners','yellow_cards','red_cards'];
const teams = {};
for (const m of map.values()) {
  for (const side of ['home','away']) {
    const name = m[side]; if (!name) continue;
    teams[name] ||= { team: name, snapshots: 0, dominant_count: 0 };
    for (const x of m.match_memory?.snapshots || []) {
      teams[name].snapshots++;
      if (x.dominant === side) teams[name].dominant_count++;
      statFields.forEach(field => {
        const value = x.stats?.[`${side}_${field}`];
        if (value === null || value === undefined) return;
        teams[name][`${field}_sum`] = (teams[name][`${field}_sum`] || 0) + value;
        teams[name][`${field}_count`] = (teams[name][`${field}_count`] || 0) + 1;
      });
    }
  }
}
Object.values(teams).forEach(t => {
  statFields.forEach(field => {
    const sum = t[`${field}_sum`] || 0;
    const count = t[`${field}_count`] || 0;
    t[`avg_${field}`] = count ? +(sum / count).toFixed(2) : null;
    delete t[`${field}_sum`];
    delete t[`${field}_count`];
  });
});
