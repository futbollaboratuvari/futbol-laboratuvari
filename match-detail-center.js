(() => {
  ...
  const recentHtml = (detail) => detail.recent_matches?.available ? `<div class="flmd-grid">${recentList(detail.home, detail.recent_matches.home)}${recentList(detail.away, detail.recent_matches.away)}${sources(detail)}</div>` : empty("Takımların doğrulanmış son maç verisi henüz bulunmuyor.");
