...
- Baslangic bulgusu: Learning memory kontrolunde once 1096, daha sonra otomatik pipeline ilerledikce 976 adet 2026-09-15..19 tarihli eski pending tahmin tespit edildi. 976 eski pending'in 641'i robot arsivinde exact mac kimligiyle bulunabiliyordu; 236 benzersiz resmi Iddaa event ID'sine sahipti ancak sadece 5 eslesen arsiv satirinda skor vardi. Darbogaz final skor kaynagindan cok resmi event-ID detail backfill kapasitesi ve isim kisaltmalarinda exact identity bagimliligiydi.
- Sonuc sync kapasitesi: RESULT_DATE_LIMIT varsayilani 3 -> 7 (ust sinir 14), yeniden kontrol varsayilani 120 -> 60 dakika. IDDAA_RESULT_DETAIL_LIMIT varsayilani 24 -> 60 (ust sinir 120), detail cooldown 360 -> 180 dakika. Eszamanlilik 4 olarak korundu.
- Ek sonuc kaynagi: SofaScore final/yariskor fallback varsayilan acik hale getirildi; RESULT_ENABLE_SOFASCORE=0 ile kapatilabilir. Kaynak hatasi warning'dir; skor uydurulmaz ve fail-closed korunur.
- Resmi event-ID eslestirme: Archive identity index exact anahtari korur. Exact bulunamazsa yalniz ayni/erken-gece aday tarihleri icinde home>=0.78, away>=0.78, toplam>=0.88 strict fuzzy aday kabul edilir; farkli event-ID rakibi skor farki 0.04'ten azsa eslesme reddedilir. Belirsiz kimlikte detail sorgusu yapilmaz.
- Gelecek ogrenme kaydi: learning-memory tahmini match_code, iddaa_event_id/official_event_id/event_id varsa, fixture/provider ID varsa, independent_evidence, include_in_coupon ve prediction-time premium eligibility bilgisini kaydeder. Boylece ileride premium performans ham aday performansindan ayrik olculebilir.
- Accuracy-first premium policy: Alt oran esigi 1.45 korunur. Value kapisi oran bandina gore sertlestirildi: 1.45-1.59 icin edge>=8, EV>=1.06, model>=76, completeness>=68; 1.60-1.89 icin 6/1.05/73/65; 1.90-2.49 icin 5/1.05/71/62; 2.50-3.49 icin 6/1.07/73/65; 3.50+ icin 8/1.10/76/70. Veri kapsami artik valueQuality'nin dogrudan parcasi.
- PRO ready fallback: Eski 60 model / 35 completeness gevsek fallback kaldirildi. Yeni fallback model>=70, completeness>=60 ve ayni valueQuality kapisini gecmek zorunda. Watch gorunumu devam eder ancak premium guclu secim sayilmaz.
- Gercek guncel veri simulasyonu: 549 robot-analysis macinda eski include_in_coupon sayisi 8 idi. Accuracy-first sayisal kapida 5 aday kaldi; eski 8 adaydan 4'u yeni standardi gecemedi. Bu secicilik bilincli davranistir.
- Geriye donuk kalite simulasyonu: O anki learning-memory'de 530 settled tahmin genel olarak %53.4 isabet ve %-7.4 flat ROI gosteriyordu. Historical independent_evidence flag eski kayitlarda olmadigi icin yalniz sayisal kalite filtresi uygulanarak yapilan retrospektif simulasyonda 11 secim kaldi; 7/11 (%63.6) dogru ve flat ROI pozitif gorundu. Orneklem 11 oldugu icin production basari iddiasi veya garanti olarak kullanilmaz; yalniz accuracy-first yonunun kontrol kanitidir.
- Test: Robot Learning CI run 35490179284 Node 20/24 success. PRO Market Specialist CI run 35490179233 Node 20/24 success. Settlement, result-ID backfill, retention, confidence, loss-pattern, specialist, PRO index, value-quality ve pre-match regresyonlari birlikte gecti.
...
- Etkilenen dosyalar: `scripts/market-specialist-gates.js`, `scripts/pro-market-specialist-postprocess.js`, `tests/market-specialist-gates.test.js`, `.github/workflows/high-odds-htft.yml`, `.github/workflows/pro-market-specialist-ci.yml`, `.github/workflows/pro-market-specialist-refresh.yml`. Korunan bülten/UI dosyalarına dokunulmadı.
...
- Geliştirme dalı / PR: `feat/pro-specialist-v4`, PR #63; ana V4 merge commit'i `5fa6b4a8`. HTFT canlı workflow run `35294721424` başarıyla tamamlandı ve `data/high-odds-htft.json` içinde `market-specialist-gates-v4` doğrulandı.
- Kalıcı gol uzman yenilemesi: V4 merge sonrasında `robot-analysis.json` gol uzman etiketi V3 kaldığı için kök neden workflow tetik kapsamı olarak belirlendi. `PRO Market Specialist Refresh` artık specialist kod/test/workflow değişikliklerinin `main` push'unda da çalışır; ürettiği salt data commitleri bu push filtresini tekrar tetiklemez.
...
### 2026-09-20 - Maç İstatistik Veri Entegrasyonu V2

- Amaç/kök neden: Maç İstatistik Merkezi V1 yalnız mevcut yerel sinyalleri birleştiriyordu. Puan tablosu kaynağı fiilen manuel dosyaya bağlı olduğu için birçok maçta played=0/rank=null kalıyor, player-intelligence çıktısı API anahtarı olmayan eski no_key durumunda kalabiliyor ve resmi İddaa eşleştirmesinin ürettiği iddaa_event_id build-full-bulletin normalizasyonunda düşürülüyordu. Bu nedenle Nesine istatistik maç kimliği sonraki veri katmanına ulaşmıyor; Özet, Puan Tablosu, Rekabet Geçmişi, Son Maçlar, Kadrolar, Korner & Kart ve Hakem alanlarının önemli bölümü veri bekliyor durumunda kalıyordu.
- Kimlik zinciri düzeltmesi: scripts/build-full-bulletin.js artık iddaa_event_id, official_event_id ve nesine_stats_id alanlarını korur. Hızlı bülten yenilemesinde yeni snapshot resmi kimliği taşımıyorsa aynı tarih/saat/ev/deplasman kimliğine sahip önceki full-bulletin kaydından resmi kimlik korunur; kısa Maçkolik matchCode değeri Nesine kimliği olarak tahmin edilmez.
- Yeni kaynak adaptörü: scripts/nesine-match-detail-source.js eklendi. Yalnız açık 6-12 haneli resmi etkinlik kimlikleriyle istatistik.nesine.com kaynağına gider. Ana /p1/{id} sayfası alındıktan sonra ev ve deplasman takım kimliği sayfa içeriğiyle doğrulanmadan hiçbir veri kabul edilmez. Kadrolar, Korner & Kart ve Hakem için ilgili özel rota ayrıca denenir; istek/cache bütçesi ile gereksiz tekrarlar engellenir.
- Veri işleme: Puan tablosu satırları ve uygun H2H tablo satırları yapılandırılmış veriye çevrilebilir. Yapılandırmaya güvenilemeyen ancak kimliği doğrulanmış bölüm verisi source_excerpt olarak tutulur. Bu metin PRO skoruna sayısal gerçekmiş gibi sokulmaz; kullanıcı arayüzünde doğrulanmış kaynak verisi olarak gösterilebilir. Eksik veri uydurulmaz.
- Merkez bağlantısı: scripts/build-match-detail-center.js yeni Nesine kaynak çıktısını mevcut standings/lineup/team-status/player/archive kaynaklarının önüne yalnız doğrulanmış yapılandırılmış alanlarda alır. Yapılandırılmış veri yoksa mevcut güvenilir fallback korunur; source_excerpt yedi sekmeye taşınır. Provenance içine Nesine kaynak adı ve doğrulanan kaynak URL'si eklenir.
- Arayüz: match-detail-center.js yapılandırılmış satır yokken source_excerpt varsa boş ekran yerine doğrulanmış kaynak bölümünü gösterir. Böylece kaynakta veri bulunmasına rağmen yerel parser henüz tam tabloya çeviremediğinde kullanıcı tamamen boş kart görmez.
- Workflow: update-fixtures içinde Nesine detay adaptörü hem ilk derived-intelligence turunda hem final bülten senkronunda match-detail-center buildinden hemen önce çalışır. Kaynak geçici olarak erişilemezse ana veri workflow'u kırılmaz; eski/doğrulanmış kaynaklar fail-closed devam eder ve health çıktısı kaynak durumunu kaydeder.
- Sağlık/çıktı: data/match-detail-source-health.json kapsama sayılarını; data/nesine-match-detail-source.json doğrulanmış normalize kayıtları; data/nesine-match-detail-cache.json kontrollü cache'i tutar. Bu dosyalar workflow veri çıktılarıyla birlikte yayınlanabilir.
- Etkilenen PRO davranışı: Market kimliği, oran, tahmin yüzdesi, value/edge, kupon eşikleri veya specialist kararları değiştirilmedi. Yalnız maç bağlamı/veri erişimi genişletildi; yapılandırılamayan kaynak metni tahmin girdisi yapılmadı.
- Test: tests/nesine-match-detail-source.test.js resmi kimlik kapısını, kısa matchCode reddini, genel "Takım" başlığının yanlış kimlik kanıtı sayılmamasını, takım kimliği doğrulamasını, resmi ID'nin full-bulletin normalizasyonunda korunmasını, puan tablosu/H2H parserini ve bölüm excerptlerini kilitler. tests/match-detail-center.test.js dış doğrulanmış kaynak önceliğini ve provenance taşımasını test eder. Match Detail Data CI run 35492941440 Node 20 ve Node 24 üzerinde syntax + iki hedef regresyon paketini success bitirdi. Aynı head üzerinde PRO Market Specialist CI run 35492941445 ve Robot Learning CI run 35492941522 de success tamamlandı.
