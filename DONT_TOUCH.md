...
- IY/MS duzeltmesi: `scripts/generate-high-odds-htft.js` ayni gun baslamis maclari Istanbul saatine gore analiz ve ilk 3 secim havuzuna almadan eler; boylece baslamis mac yeni kart slotunu tuketmez. `high-odds-htft-widget.js` de ikinci guvenlik katmani olarak baslamis kartlari gizler. HTFT JSON dogrudan `no-store` + cache-bust ile okunur, istemci 5 dakikada bir yeniden kontrol eder.
...
- Test: `tests/kupon-assistant.test.js` eklendi; `tests/high-odds-htft.test.js` tazelik korumalariyla genisletildi. `pro-market-specialist-ci.yml` PR uzerinde iki istemci dosyasini `node --check` ile ve iki regresyon testini Node 20/24 matrisinde calistiracak sekilde genisletildi. Main birlestirmesi sonrasi GitHub Actions ve canli GitHub Pages dogrulamasi tamamlanmadan is kapatilmamis sayilir.
- Etkilenen dosyalar: `nesine-coupon-assistant.js`, `scripts/generate-high-odds-htft.js`, `high-odds-htft-widget.js`, `cache-version.js`, `index.html`, `.github/workflows/high-odds-htft.yml`, `.github/workflows/pro-market-specialist-ci.yml`, `tests/high-odds-htft.test.js`, `tests/kupon-assistant.test.js`.
