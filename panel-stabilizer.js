(() => {
  const panels = [
    ...
    { id: "robot-analizleri", title: "Kupon Merkezi", text: "Dengeli, yüksek oranlı ve riskli laboratuvar kuponları" },
    { id: "membership-payment-panel", title: "Üyelik / Satın Al", text: "Gold, Diamond ve Premium paketler" },
  ...
  const cleanLegacyHeaderAccess = () => {
    const header = document.querySelector(".site-header");
    if (!header) return;
    header.querySelectorAll(".fl-access-actions, .fl-access-flow-note, .fl-access-button").forEach((node) => node.remove());
    header.querySelectorAll("button, a, div, span").forEach((node) => {
      const text = String(node.textContent || "").toLocaleLowerCase("tr-TR");
      if (text.includes("giriş yap") || text.includes("1 gün dene") || text.includes("1 gün ücretsiz dene")) node.remove();
    });
  };
  ...
  const ensureSwitcher = () => {
    ...
    switcher.innerHTML = `<div class="fl-panel-switcher-head"><div><strong>Canlı Veri Panelleri</strong><br><span>Paneller ayrı ayrı açılır.</span></div></div><div class="fl-panel-buttons"></div>`;
    const anchor = hero();
