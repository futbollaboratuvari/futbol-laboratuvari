(() => {
  ...
  const detailOdds = (match) => {
    ...
    const totals = section("Alt / Üst",
      market("0.5 Alt", pick(match, ["under05", "alt05", "under05_guess"])) +
      market("0.5 Üst", pick(match, ["over05", "ust05", "over05_guess"])) +
      market("1.5 Alt", pick(match, ["under15", "alt15", "under15_guess"])) +
    ...
    const halfTotals = section("Yarı Alt / Üst",
      market("1Y 0.5 Alt", pick(match, ["firstHalfUnder05", "iyAlt05", "firstHalfUnder05_guess"])) +
      market("1Y 0.5 Üst", pick(match, ["firstHalfOver05", "iyUst05", "firstHalfOver05_guess"])) +
      market("1Y 1.5 Alt", pick(match, ["firstHalfUnder15", "iyAlt15", "firstHalfUnder15_guess"])) +
      market("1Y 1.5 Üst", pick(match, ["firstHalfOver15", "iyUst15", "firstHalfOver15_guess"])) +
      market("2Y 0.5 Alt", pick(match, ["secondHalfUnder05", "ikinciYariAlt05", "secondHalfUnder05_guess"])) +
      market("2Y 0.5 Üst", pick(match, ["secondHalfOver05", "ikinciYariUst05", "secondHalfOver05_guess"])) +
      market("2Y 1.5 Alt", pick(match, ["secondHalfUnder15", "ikinciYariAlt15", "secondHalfUnder15_guess"])) +
    ...
    const teamGoals = section("Takım Golü",
      market("Ev 1.5 Alt", pick(match, ["homeUnder15", "evAlt15", "homeUnder15_guess"])) +
      market("Ev 1.5 Üst", pick(match, ["homeOver15", "evUst15", "homeOver15_guess"])) +
      market("Dep 1.5 Alt", pick(match, ["awayUnder15", "depAlt15", "awayUnder15_guess"])) +
      market("Dep 1.5 Üst", pick(match, ["awayOver15", "depUst15", "awayOver15_guess"])),
      "triple"
    );

    const ranges = section("Gol Aralığı",
      market("0-1 Gol", pick(match, ["goalRange01", "goalRange01_guess"])) +
      market("2-3 Gol", pick(match, ["goalRange23", "goalRange23_guess"])) +
      market("4-5 Gol", pick(match, ["goalRange45", "goalRange45_guess"])) +
      market("6+ Gol", pick(match, ["goalRange6Plus", "goalRange6Plus_guess"])),
      "triple"
    );
  ...
  const renderRow = (match, index) => {
    ...
    return `<div class="daily-match-row" data-index="${index}" data-date="${esc(match.date || "")}" data-kickoff="${esc(match.time || "--:--")}" data-home="${esc(match.home || "Ev sahibi")}" data-away="${esc(match.away || "Deplasman")}">
      <div class="daily-match-time" data-original-time="${esc(match.time || "--:--")}">${esc(match.time || "--:--")}</div>
      ...
      <span><button class="daily-detail-button" type="button" data-detail-index="${index}" data-daily-detail-toggle="${index}" aria-expanded="false">${active}/7 Detay</button></span>
    </div>`;
  };
  ...
  document.addEventListener("click", (event) => {
    const button = event.target.closest?.("[data-detail-index], [data-daily-detail-toggle]");
    if (!button) return;
    ...
    const index = Number(button.dataset.detailIndex ?? button.dataset.dailyDetailToggle);
