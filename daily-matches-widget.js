(() => {
  ...
  function ensureRoot() {
    ...
    const updated = app.lastUpdated ? `<span class="flw-meta-chip">Güncelleme ${esc(formatTimestamp(app.lastUpdated))}</span>` : "";
    const source = app.source ? `<span class="flw-meta-chip">Kaynak ${esc(app.source)}</span>` : "";
    root.innerHTML = `
      <div class="flw-top">
        <div class="flw-title"><span class="flw-kicker">MAÇ VE ORAN MERKEZİ</span><h2>Futbol Bülteni</h2><p>Günün programı, canlı karşılaşmalar ve resmi marketler tek ekranda.</p></div>
        <div class="flw-overview" aria-label="Bülten özeti"><span><b>${app.bulletin.length}</b> Başlamamış</span><span class="is-live"><b>${app.live.length}</b> Canlı</span><span><b>${app.finished.length}</b> Sonuç</span>${early ? `<span><b>${early}</b> Gece / Erken</span>` : ""}</div>
        <div class="flw-meta">${updated}${source}</div>
      </div>
      ${warning}
  ...
  function style() {
    ...
    s.textContent += `.flw{overflow:hidden;border-radius:22px;background:#06151f;border-color:rgba(230,184,54,.42);box-shadow:0 24px 70px rgba(0,0,0,.28)}.flw-top{display:grid;grid-template-columns:minmax(260px,1fr) auto;gap:14px 28px;align-items:end;padding:24px 26px;background:radial-gradient(circle at 85% 10%,rgba(18,139,104,.28),transparent 34%),linear-gradient(135deg,#040b14 0%,#071925 58%,#082a2a 100%);color:#fff;border-bottom:1px solid rgba(230,184,54,.34)}.flw-title h2{margin:4px 0 5px;font-size:clamp(24px,3vw,38px);line-height:1}.flw-title p{margin:0;color:#a8bbc6;font-size:12px}.flw-kicker{display:block;color:#e6b836;font-size:10px;font-weight:1000;letter-spacing:.2em}.flw-overview{display:flex;justify-content:flex-end;gap:8px;flex-wrap:wrap}.flw-overview>span{display:grid;gap:1px;min-width:92px;padding:8px 11px;border:1px solid rgba(255,255,255,.12);border-radius:12px;background:rgba(255,255,255,.055);color:#a8bbc6;font-size:9px;font-weight:900;text-transform:uppercase;letter-spacing:.06em}.flw-overview b{color:#fff;font-size:17px}.flw-overview .is-live{border-color:rgba(22,205,140,.38);background:rgba(12,126,91,.16)}.flw-overview .is-live b{color:#39e3a7}.flw-meta{grid-column:1/-1;display:flex;gap:7px;flex-wrap:wrap}.flw-meta-chip{padding:5px 9px;border-radius:999px;background:rgba(230,184,54,.09);border:1px solid rgba(230,184,54,.2);color:#d5cda9;font-size:9px;font-weight:850}.flw-tabs{padding:12px 18px;background:#07121d;border-bottom:1px solid rgba(255,255,255,.08)}.flw-tabs button{border-color:rgba(255,255,255,.12);background:#0b202c;color:#c6d4db;padding:9px 15px}.flw-tabs button:hover{border-color:rgba(230,184,54,.55);color:#fff}.flw-tabs button.on{background:linear-gradient(135deg,#d8a91c,#f0d24f);color:#07151d;box-shadow:0 7px 18px rgba(216,169,28,.18)}.flw-filter{padding:12px 18px;background:#0a1b27;border-bottom:1px solid rgba(255,255,255,.08)}.flw-filter input,.flw-filter select{border:1px solid rgba(255,255,255,.12);background:#f4f8f6;color:#09262a}.flw-filter button,.flw-detail-toggle,.flw-more{background:linear-gradient(135deg,#d8a91c,#f0d24f);color:#07151d}.flw-layout{background:#0a2e31}.flw-main{padding:14px}.flw-table{box-shadow:0 10px 32px rgba(0,0,0,.14)}.flw-head{background:linear-gradient(90deg,#071925,#0c3d3a);color:#f3d772}.flw-league{background:#dcebe4;border-left:3px solid #d8a91c}.flw-row:hover,.flw-row:focus,.flw-row.is-open{background:#edf6f1}.flw-detail-row{padding:14px;background:#e9f2ed}.flw-detail-grid{grid-template-columns:repeat(3,minmax(0,1fr));gap:10px}.flw-detail-card{border-color:#c5d8cf;border-radius:12px;background:#f8fcfa;padding:12px}.flw-detail-summary{grid-column:1/-1;background:linear-gradient(135deg,#f9fcfa,#eef7f2);border-color:#aecdc0}.flw-detail-summary>b{font-size:13px}.flw-detail-summary .flw-detail-note{margin-top:9px;padding-top:8px;border-top:1px solid #d7e4de}.flw-detail-market{margin:0;padding:7px 0;border-bottom:1px solid #dce8e2}.flw-detail-market:last-of-type{border-bottom:0}.flw-detail-all{background:#fff}.flw-market-section h4{background:linear-gradient(90deg,#08232d,#0a504b)}.flw-slip{background:linear-gradient(180deg,#071925,#092d2d)}@media(max-width:920px){.flw-top{grid-template-columns:1fr;padding:20px 16px}.flw-overview{justify-content:flex-start}.flw-meta{grid-column:1}.flw-detail-grid{grid-template-columns:1fr}.flw-detail-summary{grid-column:1}.flw-tabs,.flw-filter{padding-left:12px;padding-right:12px}}`;
    // Scope the complete bulletin theme so later legacy market styles cannot undo it.
    s.textContent += `
      #daily-matches-widget{color-scheme:dark}
      #daily-matches-widget .flw-layout{background:#07121d}
      #daily-matches-widget .flw-table{background:#0b1b28;color:#e6edf2;border:1px solid #263947}
      #daily-matches-widget .flw-head{background:#081522;color:#f0cc62}
      #daily-matches-widget .flw-head span,#daily-matches-widget .flw-row>*{border-color:#263947}
      #daily-matches-widget .flw-row{background:#0b1b28;color:#e6edf2}
      #daily-matches-widget .flw-row:hover,#daily-matches-widget .flw-row:focus,#daily-matches-widget .flw-row.is-open{background:#12332f}
      #daily-matches-widget .flw-row:focus-visible{outline:2px solid #e6b836;outline-offset:-2px}
      #daily-matches-widget .flw-league{background:#142834;color:#f0cc62;border-left:3px solid #e6b836;padding:11px 12px}
      #daily-matches-widget .flw-time{color:#62e0b3}
      #daily-matches-widget .flw-live-time{color:#ff9696}
      #daily-matches-widget .flw-finished-time,#daily-matches-widget .flw-code{color:#acbeca}
      #daily-matches-widget .flw-teams b{color:#f0f5f8;font-size:12px;line-height:1.5}
      #daily-matches-widget .flw-odd{background:#122c3a;border-color:#375766;color:#f1f7fa;min-height:36px;font-variant-numeric:tabular-nums}
      #daily-matches-widget .flw-odd:hover:not(:disabled),#daily-matches-widget .flw-odd.on{background:#e6b836;border-color:#e6b836;color:#07121d}
      #daily-matches-widget .flw-odd:disabled{background:#101e28;color:#8196a3;border-color:#293c48}
      #daily-matches-widget button:focus-visible,#daily-matches-widget input:focus-visible,#daily-matches-widget select:focus-visible{outline:2px solid #f0cc62;outline-offset:2px}
      #daily-matches-widget .flw-filter input,#daily-matches-widget .flw-filter select{background:#102532;border:1px solid #37515f;color:#e6edf2}
      #daily-matches-widget .flw-filter input::placeholder{color:#acbeca}
      #daily-matches-widget .flw-detail-row{background:#07121d;border-color:#263947}
      #daily-matches-widget .flw-detail-card,#daily-matches-widget .flw-detail-summary,#daily-matches-widget .flw-detail-all{background:#10222f;border-color:#2e4755;color:#dce6ec}
      #daily-matches-widget .flw-detail-card>b{color:#f0cc62}
      #daily-matches-widget .flw-detail-card b{color:#e6edf2}
      #daily-matches-widget .flw-detail-note,#daily-matches-widget .flw-market-heading span{color:#acbeca}
      #daily-matches-widget .flw-detail-summary .flw-detail-note,#daily-matches-widget .flw-detail-market{border-color:#2e4755}
      #daily-matches-widget .flw-detail-loading{border-color:#375766;color:#b9d7cf}
      #daily-matches-widget .flw-market-block{background:#0b1b28;border-color:#2e4755}
      #daily-matches-widget .flw-market-title{color:#e6edf2}
      #daily-matches-widget .flw-market-info{background:#214339;color:#8be4bd}
      #daily-matches-widget .flw-market-section h4{background:#16372f;color:#b7f2d6}
      #daily-matches-widget .flw-market-outcome b{color:inherit}
      #daily-matches-widget .flw-detail-bridge-odd{background:#122c3a;border-color:#375766;color:#e6edf2;min-height:40px}
      #daily-matches-widget .flw-detail-bridge-odd strong{color:#8be4bd}
      #daily-matches-widget .flw-detail-bridge-odd:hover{background:#e6b836;color:#07121d}
      #daily-matches-widget .flw-detail-bridge-odd:hover strong{color:#07121d}
      #daily-matches-widget .flw-detail-bridge-note{color:#acbeca}
      #daily-matches-widget .flw-slip{background:#0a1b25;color:#e6edf2}
      @media(max-width:640px){
        ...
        #daily-matches-widget .flw-row>:nth-child(6)::before{content:'2'}
        #daily-matches-widget .flw-row>:nth-child(7)::before{content:'Alt'}
        ...
        #daily-matches-widget .flw-league{border-radius:8px;margin-top:14px}
      }
    `;
  ...
  function detailOddLine(m) {
    const lines = detailMarkets.map(([key, label, keys]) => detailOddBtn(m, key, label, keys)).filter(Boolean);
    const marketHtml = lines.length ? lines.join("") : `<div class="flw-detail-note">Bu maç için doğrulanmış ek oran henüz bulunmuyor.</div>`;
    return `${marketHtml}<div class="flw-detail-note">Oran kaynağı: ${esc(m.oddsSource || m.source || "Maçkolik veri akışı")}. ${lines.length ? `${lines.length} geçerli market gösteriliyor.` : "Resmi akışın güncellenmesi bekleniyor."}</div>`;
  }
  ...
  function detailHtml(m) {
    ...
    return `<div class="flw-detail-row"><div class="flw-detail-grid">
      <div class="flw-detail-card flw-detail-summary"><b>Özet Oranlar</b>${detailOddLine(detail)}</div>
